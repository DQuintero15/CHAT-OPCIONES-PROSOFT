...
const WEBHOOK_BASE =
  "https://demos-n8n-webhook.ykgdwx.easypanel.host/webhook/CON?chatInput=";
...
function renderTypingBubble() {
  ...
  const bubble = document.createElement("div");
  bubble.className =
    "max-w-[80%] bg-gray-100 text-gray-900 px-4 py-3 rounded-xl rounded-bl-none whitespace-pre-wrap text-sm flex gap-2 items-center";
  bubble.innerHTML =
...
messagesEl.appendChild(
  createMessageEl({
    role: "assistant",
    text: 'Hola — pregunta por una opción o por "Kardex" para probar el demo.',
  })
);
scrollToBottom();
